...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* ----- AI path: workflow JSON generated by the Rostr Deploy Engineer ----- */
    if (b.customWorkflow) {
      let wf;
      try { wf = JSON.parse(String(b.customWorkflow)); }
      catch { return res.status(400).json({ error: 'The generated workflow was not valid JSON.' }); }
      if (!wf || !Array.isArray(wf.nodes) || wf.nodes.length === 0) {
        return res.status(400).json({ error: 'The generated workflow had no nodes.' });
      }
      const ctx = sanitizeCtx(b);
      if (!ctx.email || !ctx.email.includes('@')) {
        return res.status(400).json({ error: 'A valid email is required for your receipt.' });
      }
      const meta = {
        label: 'AI Deploy Engineer package',
        file: 'workflow.json',
        blurb: 'Custom n8n workflow generated by the Salesgency Deploy Engineer from your interview answers.',
      };
      const fileSlug = slug(ctx.company);
      const zip = zipFiles([
        { name: `${fileSlug}-${meta.file}`, data: Buffer.from(JSON.stringify(wf, null, 2), 'utf8') },
      ...
      return res.status(200).json({
        status: 'success',
        filename: `${fileSlug}-automation-package.zip`,
        engine: meta.label,
        company: ctx.company,
