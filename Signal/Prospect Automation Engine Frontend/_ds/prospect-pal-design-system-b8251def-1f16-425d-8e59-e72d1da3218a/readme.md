...
| **Workspace app** | Authenticated shell: dashboard, **intake wizard** (typeform-style, one question per screen, every answer versioned), **campaigns** (card grid + search; a campaign holds its outputs, deploy and history), **workflow editor** (white n8n-style canvas per campaign), **agents** (configure tools · write copy · generate workflow · edit workflow), tech signals, integrations & keys | `src/components/Sidebar.tsx`, `TopBar.tsx`, `src/components/views/*` + the 8.25.26 product notes |
...
- `brand/` — `Logo`
- `app/` — `NavItem`, `StatTile`, `ChatBubble`, `TypingDots`, `ToolOptionCard`, `ToolPicker`, `QuestionStep`, `CampaignCard`, `AgentCard`, `StepIndicator`, `Modal`
- `pipeline/` — `NodeCard`, `PipelineRail`, `CanvasBoard`
- `marketing/` — `SectionHeading`, `IntegrationCard`, `DeliverableCard`, `PricingCard`, `LeadSignalCard`

**`ui_kits/`**
- `app/` — workspace recreation (dashboard, intake wizard, campaigns grid, white-canvas workflow editor, agents, campaign workspace, tech signals, integrations & keys, deploy modal)
- `marketing/` — landing page recreation (hero canvas, bindings, deliverables, signals, pricing, checkout)

### Product-model note (8.25.26 notes)
The wizard and the builder are now distinct: **intake wizard** = conversational, one question per screen, every answer posted to history so campaigns carry versions; **workflow editor** = the white n8n-replica canvas for editing a compiled campaign. "Outputs & deploy" is reframed as **Campaigns** — the campaign is the object, artifacts and deploy live inside it. Scripts Studio is gone. Tool questions offer the common choices as chips plus a free-text row: an unlisted tool still gets an integration, built from its docs.
...
- **`StepIndicator`** — the PRD defines gated intake steps but the code had no shared progress control.
- **`QuestionStep` / `ToolPicker` / `CampaignCard` / `AgentCard` / `CanvasBoard`** — required by the 8.25.26 product notes (typeform intake, any-tool selection, campaigns as objects, four build agents, white editing canvas).
